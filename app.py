...
async def createpost_receive_video(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """CreatePost အတွက် video လက်ခံခြင်း"""
    video = None
    file_name = None
    ...
    if update.message.video:
        ...
        logger.info(f"CreatePost video received (native): {video.file_id}")
        ...
        if mime.startswith('video/') or (doc.file_name and doc.file_name.lower().endswith(('.mp4', '.mkv', '.avi', '.mov', '.webm'))):
            ...
            logger.info(f"CreatePost video received (document): {doc.file_id}")
        else:
            await update.message.reply_text("❌ Video ဖိုင် (mp4, mkv, avi, mov, webm) သာ ပို့ပေးပါ။")
            return CREATE_VIDEO
    ...
    await update.message.reply_photo(photo=poster, caption=formatted_info, parse_mode='Markdown', reply_markup=InlineKeyboardMarkup(keyboard))
...
# ========== /newfile Command (no conversation, simple) ==========
...
async def newfile_receive_video(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """NewFile အတွက် video လက်ခံခြင်း"""
...
CHANNELPOST_PHOTO, CHANNELPOST_CAPTION, CHANNELPOST_VIDEO = range(3)
...
async def channelpost_receive_photo(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text("✍️ ဇာတ်ကားအချက်အလက် စာသား (သို့) ဇာတ်ညွှန်းကို ပို့ပေးပါ။")
    return CHANNELPOST_CAPTION

async def channelpost_receive_caption(update: Update, context: ContextTypes.DEFAULT_TYPE):
    context.user_data['channelpost_caption'] = update.message.text
    await update.message.reply_text("🎬 ယခု ဇာတ်ကား Video ဖိုင်ကို ပို့ပေးပါ။")
...
async def menu_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "menu_createpost":
        ...
        await query.edit_message_text("🔗 `/newfile` command ကိုသုံးပါ။")
...
# ChannelPost Conversation
channelpost_conversation = ConversationHandler(
    ...
    states={
        ...
        CHANNELPOST_CAPTION: [MessageHandler(filters.TEXT & ~filters.COMMAND, channelpost_receive_caption)],
...
application.add_handler(channelpost_conversation)
